...
export const createSubscriptionPlan = async (
  ...
  try {
    ...
    const [existingPlans] = await conn.execute<RowDataPacket[]>(
      `
      SELECT id
    ...
    if (!planId) {
      throw new Error(
        "Subscription plan created but insert ID was not returned."
      );
    }
    ...
    const [createdPlan] = await conn.execute<RowDataPacket[]>(
      `
      SELECT *
      FROM subscription_plans
      WHERE id = ?
...
export const updateSubscriptionPlan = async (
  ...
  try {
    ...
    const [existingPlan] = await conn.execute<RowDataPacket[]>(
      `
      SELECT id
      FROM subscription_plans
      WHERE id = ?
    ...
    const [duplicateCode] = await conn.execute<RowDataPacket[]>(
      `
      SELECT id
      FROM subscription_plans
      WHERE plan_code = ?
    ...
    const [updatedPlan] = await conn.execute<RowDataPacket[]>(
      `
      SELECT *
      FROM subscription_plans
      WHERE id = ?
 ...
 * ============================================================
 */
export const assignModulesToPlan = async (
  req: Request,
  res: Response
) => {
  let conn;
  ...
  try {
    ...
    const moduleIds = [
      ...new Set(
        modules
          .map((moduleId: any) => Number(moduleId))
          .filter((moduleId: number) => moduleId > 0)
      ),
    ];
