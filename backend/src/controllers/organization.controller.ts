...
export const getOrganizationById = async (
  ...
  try {
    ...
    const [orgRows] = await conn.execute<any[]>(
      `
      SELECT
          id,
          name,
    ...
    const [currentPlan] = await conn.execute<any[]>(
`
SELECT
    op.id,
...
[id]
    ...
    const [pendingRequest] = await conn.execute<any[]>(
`
SELECT
    spr.id,
...
export const getAvailableModules = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [rows] = await conn.execute<any[]>(
      `SELECT code FROM permissions WHERE deleted_at IS NULL AND is_deprecated = 0 ORDER BY code`
    );
...
export const getOrganizationModules = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [rows] = await conn.execute<any[]>(
      `SELECT id, module_code, pricing_type, price, custom_details, status, assigned_by, created_at, updated_at
...
export const assignModuleToOrganization = async (req: Request, res: Response) => {
  let conn;
  ...
  try {
    ...
    const [moduleRows] = await conn.execute<any[]>(
      `SELECT id FROM permissions WHERE code = ? AND deleted_at IS NULL AND is_deprecated = 0 LIMIT 1`,
      [module_code]
    );
    ...
    const [existingRows] = await conn.execute<any[]>(
      `SELECT id FROM organization_modules WHERE organization_id = ? AND module_code = ? AND deleted_at IS NULL LIMIT 1`,
      [organizationId, module_code]
    );
    ...
    if (existingRows.length) {
      ...
      await conn.execute(
        `UPDATE organization_modules
         ...
         WHERE id = ?`,
        [pricing_type, modulePrice, custom_details || null, status, req.user?.id || null, assignmentId]
      );
    ...
    await conn.execute(
      `INSERT INTO organization_modules
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
      [organizationId, module_code, pricing_type, modulePrice, custom_details || null, status, req.user?.id || null]
    );
...
export const assignPlanToOrganization = async (req: Request, res: Response) => {
...
export const approvePlanRequest = async (
  req: Request,
  res: Response
) => {
  let conn;
...
export const rejectPlanRequest = async (
  req: Request,
  res: Response
) => {
